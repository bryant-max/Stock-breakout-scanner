...
const FAQ = [
  ...
  { q: "What happens after the free trial?", a: `After your ${TRIAL_DAYS}-day free trial, you'll be charged the monthly rate for your chosen plan. We'll remind you before billing starts.` },
...
export default function PricingPage() {
  ...
  const handleSubscribe = async (planId: PlanId) => {
    ...
    try {
      ...
      if (!res.ok) { const b = await res.json().catch(() => ({})); throw new Error(b?.detail ?? "Checkout failed") }
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      <div className="fixed inset-0 pointer-events-none">
        <div className="absolute inset-0 opacity-[0.06]" style={{ backgroundImage: "linear-gradient(to right, #00ff88 1px, transparent 1px), linear-gradient(to bottom, #00ff88 1px, transparent 1px)", backgroundSize: "80px 80px" }} />
      </div>

      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 py-24">
        <button onClick={() => navigate("/")} className="mb-12 font-mono text-xs uppercase tracking-wider text-white/40 hover:text-[#00ff88] transition-colors">
          ← Back to Home
        </button>
        ...
        <div className="text-center mb-20">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold mb-6 leading-tight">Simple, transparent pricing.</h1>
        ...
        {error && (
          <div className="max-w-5xl mx-auto mb-8 border border-red-500/40 bg-red-500/10 px-4 py-3 font-mono text-sm text-red-400">{error}</div>
        )}

        {/* Pricing cards */}
        <div className="grid gap-8 md:grid-cols-2 max-w-5xl mx-auto mb-24">
          {PRICING_PLANS.map((plan) => (
            <div key={plan.id} className={`relative border ${plan.isPopular ? "border-[#00ff88] bg-[#00ff88]/5" : "border-[#222] bg-[#0a0a0a]"}`}>
              {plan.isPopular && (
                <>
                  <div className="absolute -top-px left-0 right-0 h-px bg-[#00ff88]" />
                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2">
                    <span className="font-mono text-[10px] uppercase tracking-wider text-black bg-[#00ff88] px-3 py-1">Most Popular</span>
                  </div>
                </>
              )}
              <div className="p-8 sm:p-10">
                ...
                <button
                  ...
                  className={`mt-8 w-full py-4 font-mono text-sm font-bold uppercase tracking-wider transition-all disabled:opacity-60 disabled:cursor-not-allowed ${plan.isPopular ? "border border-[#00ff88] bg-[#00ff88] text-black hover:bg-transparent hover:text-[#00ff88]" : "border border-white/20 text-white hover:border-[#00ff88] hover:text-[#00ff88]"}`}
                >
                    ...
                    <li key={i} className="flex items-center gap-3">
                      {feature.included ? (
                        <div className="flex h-5 w-5 shrink-0 items-center justify-center border border-[#00ff88] bg-[#00ff88]">
                          <svg className="h-2.5 w-2.5 text-black" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={4}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>
                        </div>
                      ) : (
                        <div className="flex h-5 w-5 shrink-0 items-center justify-center border border-white/20">
                          <svg className="h-2.5 w-2.5 text-white/20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={4}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                        </div>
                      )}
                      <span className={`font-mono text-sm ${feature.included ? "text-white" : "text-white/30 line-through"}`}>{feature.text}</span>
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <div className="max-w-5xl mx-auto mb-24">
          <h2 className="font-mono text-xs uppercase tracking-wider text-white/40 text-center mb-8">Full Comparison</h2>
            ...
            <div className="grid grid-cols-3 bg-[#0a0a0a] border-b border-[#222]">
              ...
              {PRICING_PLANS.map((plan) => (
                <div key={plan.id} className="p-4 text-center">
                  <span className={`font-mono text-sm font-bold ${plan.isPopular ? "text-[#00ff88]" : "text-white"}`}>{plan.name}</span>
                  <div className="font-mono text-xs text-white/40">${plan.price}/mo</div>
                </div>
              ))}
            </div>
            {PRICING_PLANS[0].features.map((feature, i) => (
              <div key={i} className={`grid grid-cols-3 border-b border-[#111] ${i % 2 === 0 ? "bg-black" : "bg-[#050505]"}`}>
          ...
          <div className="flex items-center justify-center gap-4 flex-wrap">
            {PRICING_PLANS.map((plan) => (
              <button
                ...
                onClick={() => handleSubscribe(plan.id)}
                disabled={!!loadingPlan}
                className={`px-8 py-4 font-mono text-sm font-bold uppercase tracking-wider transition-all disabled:opacity-60 ${plan.isPopular ? "border border-[#00ff88] bg-[#00ff88] text-black hover:bg-transparent hover:text-[#00ff88]" : "border border-white/30 text-white hover:border-[#00ff88] hover:text-[#00ff88]"}`}
              >
                {loadingPlan === plan.id ? "Redirecting…" : `${plan.name} — $${plan.price}/mo`}
              </button>
            ))}
          </div>
        </div>
